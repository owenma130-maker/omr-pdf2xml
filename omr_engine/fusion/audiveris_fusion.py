"""
Audiveris .omr文件解析 → MusicXML导出 + PS真值评估
...
def load_ps_truth(ps_path):
    """加载PS真值MusicXML, 返回音符列表 [(midi, type, dur)]"""
    ...
    for note in root.iter('note'):
        ...
        midi = 12 * (octv + 1) + STEP_TO_SEMITONE[step] + alter
...
def load_pred_notes(musicxml_path):
    """加载预测MusicXML, 返回音符列表 [(midi, type, dur)]"""
    return load_ps_truth(musicxml_path)  # 格式相同


def dtw_align(pred, truth):
    """DTW对齐: 返回pred索引→truth索引映射"""
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            cost = 0 if pred[i - 1][0] == truth[j - 1][0] else 1
    ...
    while i > 0 and j > 0:
        if dp[i][j] == dp[i - 1][j - 1] + (
                0 if pred[i - 1][0] == truth[j - 1][0] else 1):
...
def evaluate(pred_path, truth_path):
    """评估预测vs真值"""
    pred = load_pred_notes(pred_path)
    truth = load_ps_truth(truth_path)
    print(f'预测: {len(pred)} 音符, PS真值: {len(truth)} 音符')

    # DTW对齐
    align = dtw_align(pred, truth)
    if not align:
        print('  DTW对齐失败!')
        return None
    ...
    matched = 0
    for pi, ti in align:
        if pred[pi][0] == truth[ti][0]:
            matched += 1
            truth_matched.add(ti)
    precision = len(truth_matched) / len(truth) * 100

    print(f'  音高对账: {len(align)}/{len(truth)} '
          f'({len(align) / len(truth) * 100:.0f}%), 匹配{matched}')
    print(f'  音高精确率(真值覆盖): {precision:.1f}% '
          f'({len(truth_matched)}/{len(truth)})')

    # 时值准确率
    dur_ok = sum(1 for pi, ti in align if pred[pi][1] == truth[ti][1])
    dur_acc = dur_ok / len(align) * 100 if align else 0
    print(f'  时值准确率: {dur_acc:.1f}%')

    # 音高+时值都对
    both_ok = sum(1 for pi, ti in align
                  if pred[pi][0] == truth[ti][0] and pred[pi][1] == truth[ti][1])
    if align:
        print(f'  音高+时值都对: {both_ok}/{len(align)} '
              f'({both_ok / len(align) * 100:.1f}%)')

    # 召回率
    recall = len(align) / len(truth) * 100
    print(f'  音符召回率: {recall:.1f}% (对账{len(align)}/{len(truth)})')

    # 时值分布对比
    pred_durs = Counter(d for _, d, _ in pred)
    truth_durs = Counter(d for _, d, _ in truth)
    print(f'  预测时值: {dict(pred_durs.most_common())}')
    print(f'  PS真值时值: {dict(truth_durs.most_common())}')
...
# ============================================================
# 7. 主流程: 解析OMR → 转换 → 导出 → 评估
# ============================================================

def run_pipeline(omr_path=None, output_path=None, truth_path=None,
                 mxl_path=None, verbose=True):
    """
    完整流程: 解析OMR → 音高转换 → 时值推断 → 导出MusicXML → 评估

    Args:
        omr_path: .omr文件路径
        output_path: 输出MusicXML路径
        truth_path: PS真值路径
        mxl_path: Audiveris导出的MXL路径(用于提取已知时值)
        verbose: 是否打印详细信息

    Returns:
        (notes, eval_result)
    """
    omr_path = omr_path or str(DEFAULT_OMR)
    output_path = output_path or str(DEFAULT_OUTPUT)
    truth_path = truth_path or str(DEFAULT_TRUTH)

    if verbose:
        print(f'[1/5] 解析OMR: {omr_path}')

    # 1. 解析OMR
    raw_notes = parse_omr(omr_path)
    if verbose:
        sheets = Counter(n['sheet'] for n in raw_notes)
        print(f'  检测到 {len(raw_notes)} 个音符头')
        for s in sorted(sheets.keys()):
            print(f'    页{s}: {sheets[s]} 个')

    # 2. 加载MXL已知时值(如果有)
    mxl_notes = []
    mxl_durations = {}
    if mxl_path and os.path.exists(mxl_path):
        mxl_notes = load_mxl_durations(mxl_path)
        if verbose:
            print(f'  MXL已知时值: {len(mxl_notes)} 个音符')
    ...
    converted = []
    for n in raw_notes:
        step, octave, alter = pitch_to_musicxml(n['pitch_raw'])
        n['step'] = step
        n['octave'] = octave
        # 保留OMR解析时设置的alter(来自变音记号), 若无则用pitch_to_musicxml的
        if 'alter' not in n:
            n['alter'] = alter
        converted.append(n)
    ...
    if verbose:
        print('[3/5] 时值推断...')
    for n in converted:
        note_type, dur = infer_duration(n)
        n['type'] = note_type
        n['dur'] = dur
        n['dotted'] = n.get('is_dotted', False)
        if n['dotted']:
            n['dur'] = dur * 1.5

    # 5. 导出MusicXML
    if verbose:
        print(f'[4/5] 导出MusicXML: {output_path}')
    export_musicxml(converted, output_path)

    # 6. 评估
    if verbose:
        print(f'[5/5] 评估 vs PS真值...')
    eval_result = None
    if os.path.exists(truth_path):
        eval_result = evaluate(output_path, truth_path)
    else:
        print(f'  [!] 真值文件不存在: {truth_path}')

    return converted, eval_result
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Audiveris OMR解析→MusicXML导出→评估')
    parser.add_argument('--omr', default=str(DEFAULT_OMR), help='OMR文件路径')
    parser.add_argument('--output', default=str(DEFAULT_OUTPUT), help='输出MusicXML路径')
    parser.add_argument('--truth', default=str(DEFAULT_TRUTH), help='PS真值路径')
    parser.add_argument('--mxl', default=None, help='Audiveris MXL路径(可选)')
    args = parser.parse_args()

    # 如果没有指定MXL, 尝试从OMR同目录找
    if args.mxl is None:
        omr_dir = os.path.dirname(args.omr)
        mxl_candidates = [f for f in os.listdir(omr_dir)
                          if f.endswith('.mxl') or f.endswith('.xml')]
        for c in mxl_candidates:
            if 'extracted' in c or c.endswith('.mxl'):
                args.mxl = os.path.join(omr_dir, c)
                break

    notes, result = run_pipeline(
        omr_path=args.omr,
    ...
    if result:
        print(f"\n=== 总结: 音高{result['pitch_acc']:.1f}% "
              f"时值{result['dur_acc']:.1f}% "
              f"召回{result['recall']:.1f}% ===")
        print(f"  检测{result['n_pred']}音符, "
              f"真值{result['n_truth']}音符, "
              f"对账{result['n_aligned']}, "
              f"匹配{result['n_matched']}")
